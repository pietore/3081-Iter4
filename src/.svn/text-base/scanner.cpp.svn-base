...
Token* Scanner::scan (const char *buffer){
	...
	// Make sure that the end of file terminal
	// terminates on null
	Token* tmptokn = origTokn;
	int i = 1;
	while( tmptokn != NULL ) {
		if (tmptokn->terminal == endOfFile){
			tmptokn->next = NULL;
		}
		
		//cout << (tmptokn->terminal==endOfFile) << "\t" << i << "\t" << tmptokn->lexeme << endl;
		i++;
		tmptokn = tmptokn->next;
	}	
	
	//cout << "done scannerinng...\n\n";
...
Token* Scanner::attemptMatch (const char* buffer){
	...
	terminal_t terminal = lexicalError;
	Token* toknptr = NULL;
...
Token* makeToken(terminal_t terminal, const char* buffer, int size){
